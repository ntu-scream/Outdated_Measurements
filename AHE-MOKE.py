...
root = Tk()
root.title('AHE MOKE Measurement')

# global values are measured in thread, plotted in animation
global scan_field_output, measured_values, dataplot, curr_lbl, fix_lbl, x1, y1, x2, y2
...
def save_method(H_dir, fix_val, current_val, x_values, y_values, display, directory, m_type, name, resistance):
    ...
    file.write("\n")
    file.write("Number"+" "+H_dir+" Field(Oe)"+" "+"MOKE Signal (A.U.)"+"\n")
            ...
            for fix_val in fix_field_output:
                ...
                for current_val in current_output:
                    ...
                    for counter, scan_val in enumerate(scan_field_output):
                        ...
                        display.insert('end', 'Applied %s Field Value: %s (Oe)      MOKE Signal: %s (A.U.)' %(scan, scan_val, round(tmp, 4)))
                        display.see(END)
